<x-layout>
    ...
    <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div class="flex items-center justify-between pb-6 mb-8 border-b border-gray-200">
            <a href="{{ route('dashboard') }}" class="flex items-center hover:opacity-80 transition" title="easybuy">
                <img src="{{ asset('images/easybuy-icon.png') }}" alt="easybuy" class="h-8 w-auto object-contain block sm:hidden">
                <img src="{{ asset('images/easybuy-logo.png') }}" alt="easybuy" class="h-8 w-auto object-contain hidden sm:block">
            </a>
        ...
        <!-- Flash Messages -->
        @if(session('success'))
            <div class="mb-8 p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-md flex items-center gap-2 text-sm font-medium shadow-sm animate-fade-in">
        ...
        @if($cartItems->isEmpty())
            ...
            <div class="bg-white border border-gray-200 p-16 text-center my-8 rounded-none">
                ...
                <a href="{{ route('buyer.browse') }}" class="inline-block px-8 py-3 bg-[#f5ce42] hover:bg-[#e6c035] text-black font-semibold text-sm rounded transition shadow-sm">
                    Start Shopping &rarr;
                </a>
            </div>
        @else
                    ...
                    <div class="divide-y divide-gray-200 border-t border-b border-gray-200">
                        @foreach($cartItems as $item)
                            @php
                                ...
                                $itemImage = !empty($product->image_url) ? $product->image_url : $catalogImages[$loop->index % count($catalogImages)];
                                $itemTag = $tags[$loop->index % count($tags)];
                            @endphp

                            <div class="py-6 sm:py-8 flex gap-4 sm:gap-6 items-start justify-between group">
                                <!-- Image Thumbnail -->
                                <div class="w-24 h-24 sm:w-28 sm:h-28 bg-[#f5f5f5] flex-shrink-0 flex items-center justify-center p-2 overflow-hidden">
                                    <img src="{{ $itemImage }}" 
                                ...
                                <div class="flex-1 min-w-0 pr-2">
                                    <h3 class="text-base sm:text-lg font-bold text-gray-900 tracking-tight line-clamp-1">
                                        {{ $product->productname }}
                                    </h3>
                                    <p class="text-xs text-gray-500 italic mt-0.5 line-clamp-1">
                                        {{ $product->description }}
                                    </p>
                                    <span class="inline-block text-[10px] uppercase tracking-widest text-gray-400 font-bold mt-1">
                                    ...
                                    <div class="mt-4 flex items-center gap-4">
                                        ...
                                        <form action="{{ route('buyer.updateCart') }}" method="POST" class="inline-flex items-center border border-gray-300 rounded-sm bg-white overflow-hidden">
                                            @csrf
                                        ...
                                        <!-- Remove Link Form -->
                                        <form action="{{ route('buyer.removeFromCart') }}" method="POST" class="inline">
                                            @csrf
                                            <input type="hidden" name="cart_id" value="{{ $item->id }}">
                                            <button type="submit" class="text-xs text-gray-400 hover:text-red-600 transition cursor-pointer">
                                                Remove
                                            </button>
                                        </form>
                                    </div>

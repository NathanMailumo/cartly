    ...
    <div class="w-full max-w-md bg-white/90 backdrop-blur-md p-8 rounded-2xl shadow-xl border border-slate-100">
        ...
        <form method="post" action="{{ route('auth.reset') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" placeholder="e.g john@doe.com" name="email" value="{{ old('email') }}"
            ...
            <button type="submit"
                class="w-full py-3 px-4 bg-slate-900 hover:bg-slate-800 text-white font-semibold rounded-xl shadow-sm transition duration-150 active:scale-[0.99] text-sm mt-2">
                Send Reset Code
            </button>
        </form>
    </div>

<x-layout>
    ...
    <div class="w-full max-w-md bg-white/90 backdrop-blur-md p-8 rounded-2xl shadow-xl border border-slate-100">
        ...
        @if (session('success'))
            <div class="mb-4 p-3 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs rounded-lg text-center">
                {{ session('success') }}
            </div>
        @endif

        <h1 class="text-2xl font-bold text-center text-slate-900 tracking-tight">Enter Verification Code</h1>
        <p class="text-sm text-center text-slate-500 mt-1 mb-6">Enter the 6-digit code sent to your email</p>
